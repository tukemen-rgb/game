...
EXPECT = {
    ...
    # **索引の読み方そのものが外れている形** (#266/#267)。画面でも、この先の段が
    # 「それらしい数」を出さずに原因を指すこと (全行の突き合わせで見る)
    "length": "しか指していません",
    # **文言の入れ物だけが読めない形** (#270)。画面でも同じ判定が出ること
    "box": "文言の入れ物が 1 つも読めません",
    # **目安と本物の裏付けがぶつかる形** (#271)。画面も強いほうを採ること
    "sparse": "ゲーム自身の検査値は全部合っています",
    # **吸い出しが途中で切れた形** (#275)。画面も同じ原因を言うこと
    "halfrip": "件はゼロ埋め",
    # **0x8002 の読み方が一覧に頼っている形** (#280)。画面も 2 通りを並べること
...
BY_DESIGN_NO_TABLE = (
    ...
    # **2 通りの読みを並べる行** (#280)。CLI はフォルダの font.txt で日本語にし、
    # 画面は貼っていないので `[10][71]` の形になる。**中身が違うのではなく、
    # 持っている文字表が違う**。貼った回では突き合わせる
    ("待ち時間として:", "CLI はフォルダの font.txt で日本語にする。画面は貼っていない"),
    ("ページ送りとして:", "CLI はフォルダの font.txt で日本語にする。画面は貼っていない"),
)
