...
#: 名前の並びを「読めた」と見なす下限 (#258 → #276)。**枠どおりに読めた名前のうち、
#: 0x21〜0x7E だけでできているものの割合**。でたらめな置き場を名前と言わないための重し。
#:
#: **9 割から 3 割に下げた。** 9 割では、**本物の名前でも 1 割壊れていれば 1 つも
#: 使わなかった** —— 1,200 件の索引で測ると、壊れ 5% では 1,140 件残るのに、
#: 壊れ 11% で **0 件**。1 件の差で 1,068 件の良い名前が消える崖だった。
#:
#: 下げてよいことは測って確かめた。**枠どおりに 2 個以上読めたでたらめ**
#: (ゼロ埋め・位置表の u32 の並び・Shift-JIS の文章を 0 で区切ったもの) は
#: どれも **0〜7%** にしかならない。乱数や英文が 100% に見えるのは名前が
...
def read_dfi_names(idx: bytes, rec_end: int, rec_count: int) -> list[str]:
    ...
    **その重しは 9 割では厳しすぎた** (#276)。1,200 件の索引で測ると、壊れが
    1 割の手前では 1,140 件の名前が残るのに、**1 割で 0 件**になる。1 件の差で
    1,068 件の良い名前が消える崖だった。測り直して 3 割に下げてある
    (`NAME_CLEAN_RATIO` の但し書き)。
    """
    raw: list[bytes] = []
    q = rec_end
...
COVERAGE_MIN = 0.2
#: 診断で「索引の読み方が外れている」と言う下限 (#277)。**セクタ単位に丸めた**
#: 使い切りの割合に当てる。
#:
#: 長らく、丸めない割合 (`used / img_size`) を 2 割で見ていた。**それは
#: 「読み方が違う」ではなく「ファイルが小さい」を捕まえていた** —— 合成データで
#: 測ると、正しく読めているのに
#:
...
#:
#: となる。下の 2 つは**無事な吸い出しなのに警告が出て**、しかも #266 の仕組みで
#: この先の段が全部黙らされていた。一方、読み方が本当に違う索引
#: (`--break length`) はセクタ丸めで **30.0%** にしかならない。
#:
#: セクタに丸めれば、正しい読みは必ず 100% 近くになる (ファイルはセクタ境界に
#: 並ぶので、丸めた合計が本体を埋める)。だから線は 5 割 —— [本体] の行が前から
#: 「読み方が合っていれば普通は 5 割を超えます」と書いていた、その 5 割。
COVERAGE_SECT_MIN = 0.5
...
def check(folder: str, out=sys.stdout) -> int:
    ...
    # 言っていた (#96)。
    #
    # **判定はセクタに丸めた側でする** (#277)。ファイルはセクタ境界に並ぶので、
    # 小さいファイルばかりの吸い出しでは丸めない割合がいくらでも下がる ——
    # 16 バイトのファイルなら 0.8% で、**無事なのに読み方を疑われていた**。
    # 丸めれば正しい読みは必ず 100% 近くになる (`COVERAGE_SECT_MIN` の但し書き)
    coverage = used / max(1, img_size)
    used_sect = sum(-(-e["len"] // SECTOR) * SECTOR for e in entries)
    coverage_sect = used_sect / max(1, img_size)
    say(f"[本体] {img_size:,} バイト / 索引が指す合計 {used:,} バイト "
        f"({100 * coverage:.1f}% — 索引が本体をどれだけ使い切っているか。"
        f"セクタ ({SECTOR} バイト) に丸めると {100 * coverage_sect:.1f}%。"
        "読み方が合っていれば、丸めたほうは普通は 5 割を超えます)")
    if coverage_sect < COVERAGE_SECT_MIN:
        problems += 1
        ...
        if crc_all_ok:
            say(f"→ 索引が本体の {100 * coverage_sect:.1f}% (セクタに丸めて) しか"
                "指していません。"
    ...
    # 直したつもりで、実際に数に入れたのは MAP の段だけだった。そのほかの段
    # (文字表の 2 枚目・検査値との突き合わせ・文字表の出来具合…) は
    # 「診ていない段: 1 件」と書いた**すぐ次の行**で「問題なし。docs/10 の
    # 手順へ」と言っていた。しかも壊していない練習データでも毎回こうなる ——
    # 社長がいちばんよく見る形で、2 行が正反対のことを言っていた。
    # 吸い出しに問題があるわけではないので終了コードは 0 のまま (docs/10 へは
    # 進んでよい)。変えるのは**言い方**で、「全部を診た」と読ませない
    if problems:
        end = f"確認事項 {problems} 件 (上の → の行)。この出力ごと報告してください"
    elif skipped:
        # **数は書かない。** 「診ていない段: N 件」の行は画面と CLI で数が違う
        # (画面は文字表を貼っていないぶんが 1 つ多い) ので、締めに数を入れると
        # 2 つの報告を 1 行ずつ突き合わせられなくなる —— #175 がこの行を
        # 別の行に分けたのと同じ理由。数はすぐ上の行にある
        end = ("→ の行はありません。ただし**診ていない段があります** (すぐ上の行)。"
               "「全部を診て大丈夫」ではなく、**診た範囲では問題が無かった**という"
               "意味です。docs/10 の手順へ進めます")
    else:
        end = "問題なし。docs/10 の手順へ"
    say("\n== 結果: " + end)
