...
チェック、データへの入れ直しまでを、市販ツールと同じ手順・同じ考え方で
自分の手で動かせます。**入れ直しができるのは自作の練習データに対してだけ**です (市販ゲームの実物は読むだけ。docs/05、docs/10)。
...
> **Windows の方は先にここだけ。** この文書のコマンドは macOS / Linux の書き方です。
> Windows (コマンドプロンプト / PowerShell) では次のように読み替えてください。
> 詳しくは [docs/10 の「Windows で使うとき」](docs/10-僕夏2の手順.md)。
>
> | 書いてあるもの | Windows では |
> | --- | --- |
> | `python3 …` | `py -3 …` (`python3` と打つと Microsoft Store が開くことがあります) |
> | 行末が `\` で次の行に続く | **1 行につなげて**打ってください |
> | `… \| head -20` | `\| head -20` を**付けずに**打ってください (全部出るだけです) |
> | `… \| grep 同じ` | 同じく付けずに打って、目で探してください |
> | `cp A B` | `copy A B` (PowerShell なら `cp` のままでも動きます) |
> | `cmp A B && echo 一致` | `fc /b A B` (PowerShell の `fc` は**別のコマンド**なので `cmd /c fc /b A B`) |
>
> ファイルの区切りは `/` のままで大丈夫です。`*` を含む指定も、道具の側で探すので
> そのまま打てます。
...
10. **実物と同じ形で通す** — `make_boku2_sample.py` のデータを構造探査台と
    ...
    突き合わせる (課題 8、docs/10)
11. **わざと壊して診断を読む** — `make_boku2_sample.py --break msg` などで 10 通りに
    壊したデータを `boku2.py check` にかけ、`→` の行がどの段の外れ方かを読む (課題 9)。
    実物で最初に貼るのはこの出力 (docs/10 の「報告するとき」)

Windows で使う場合は、先に docs/10 の「Windows で使うとき」(`python3` → `py -3` の
読み替えなど) を一度だけ読んでください。
