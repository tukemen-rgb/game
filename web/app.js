 ...
 * 0.59、独自文字コードのテキストは 0.99 と、はっきり分かれます
 * (docs/07 の表と同じ数字。ずれたら tests/e2e/viewer.py の隣の検査が落ちる)。
 */
...
function diagnose() {
  ...
  /* **統計を取るには短すぎるものに、確信のある顔をさせない** (#162)。
     `blockStats` は 16 バイト未満のとき `scant` を立てるのに、誰も見ていなかった。
     そのせいで 1 バイトの `Thumbs.db` が「独自の形式です / 根拠 3 件 /
     タイル・コードなど 100%」と出る。**1 バイトから 3 つの根拠は出ない。**
     吸い出したフォルダには `.DS_Store` や `Thumbs.db` がほぼ必ず混ざるので、
     素人が最初に開いた画面でこれを見ることになる。
     魔法数は 4 バイトで決まるので、それに当たるものは除く。
     `scant` そのものは使えない —— あれは**末尾のゼロを落とした後**の長さで
     立つので、32KB のゼロ埋め (`PAD.DAT`) まで「短い」に入ってしまう。
     見るのは**ファイルそのものの長さ**。ゼロ埋めは今までどおり「ゼロ埋め」と言う */
  const tooSmall = b.length < 16 && !MAGICS.some(
    (m) => b.length >= m.bytes.length && m.bytes.every((v, i) => b[i] === v));

  if (tooSmall) {
    verdict = "小さすぎて判定できません";
    sub = "中身が " + b.length + " バイトしかないので、バイトの統計が取れません。"
      + "吸い出したフォルダに紛れ込む `.DS_Store` (Mac) や `Thumbs.db` (Windows) は"
      + "この形です。ゲームのファイルのつもりなら、16 進タブで中身を直に見てください。";
    say(`中身が ${b.length} バイト`, "統計に要るのは 16 バイト以上",
        "割合やエントロピーは、標本が少ないと意味のある値になりません。"
        + "短いものを無理に分類すると、どれも同じ結論になります。",
        "**測れないときは測れないと言う。** 数字が出るからといって、"
        + "その数字に意味があるとは限りません。");
    /* 表示側は layout / preview を必ず見るので、形を揃えて返す
       (欠けると診断タブが真っ白になる) */
    return { verdict, sub, facts, layout: [], endian, preview: "",
             next: [{ label: "16 進タブで中身を見る",
                      why: "短いので目で全部読めます" }] };
  }
  ...
  /* --- 根拠を並べる --- */
  /* 1 種類しか無いファイルでも必ず出す。以前は「2 種類以上」のときだけ出していたので、
     BGM.ADP (波形 100%) や PAD.DAT (ゼロ埋め 100%) では**いちばん強い証拠が消え**、
     上で「波形の性質があります」と言った直後に「根拠 (0) 決め手になる手がかりが
     見つかりませんでした」と出ていた。素人には矛盾に読める (#112) */
  if (dist && dist.sorted.length) {
    const parts = dist.sorted.slice(0, 5)
      .map(([k, n]) => `${CLASSES[k].label} ${Math.round((n / dist.total) * 100)}%`);
    const one = dist.sorted.length === 1;
    say("中身の内訳", parts.join(" / "),
        `ファイル全体を ${dist.total.toLocaleString("ja-JP")} 区画に分けて数えた結果です。`
        + (one ? "端から端まで同じ性質でした。" : ""),
        one
        ? "全部が 1 種類というのは、それ自体が強い手がかりです。詰め合わせではないので、"
          + "切り分けを考える必要がありません。この性質のまま読み方を探します。"
...
async function buildIdxReport() {
  ...
  if (maps.length) {
    ...
    /* **診ていない段があるなら、「問題なし」で終わらせない** (#174)。
       MAP を読んでいないと物語の会話をまるごと診ないのに、そのまま
       「問題なし。docs/10 の手順へ」と言っていた。社長は「診た結果、大丈夫」と読む */
    problems++;
    lines.push("→ MAP が無いので、**物語の会話は 1 つも診ていません** (MAP のファイルを渡していません)。"
  ...
  let end = "問題なし。docs/10 の手順へ";
  if (problems) end = `確認事項 ${problems} 件 (上の → の行)。この出力ごと報告してください`;
  else if (skipped.length) {
    /* **数は書かない** (#268)。画面は文字表を貼っていないぶん診ていない段が
       1 つ多いので、締めに数を入れると CLI と 1 行ずつ突き合わせられなくなる */
    end = "→ の行はありません。ただし**診ていない段があります** (すぐ上の行)。"
      + "「全部を診て大丈夫」ではなく、**診た範囲では問題が無かった**という意味です。"
      + "docs/10 の手順へ進めます";
  }
  lines.push("== 結果: " + end);
  return lines.join("\n");
...
function readDfi(idx, dataSize, rule) {
  /* rule: "flag" (既定。公開ソース UNPACK.py の規則。「続く」0 のフォルダで旗を立て、
           ...
           「続く」が 0 なら親も閉じる、何段でも)。
     既定を flag にしてあるのは、小さな形を 816 通り総当たりして食い違った 34 通りの
     すべてで、公開ソースの実物のコードが flag と同じ答えを出したから (#108)。
     向こうの道具は実物で動いているので、裏付けがあるのは flag の側だけ。
     報告用の要約は今も両方で道筋を作って突き合わせる */
  rule = rule || "flag";
...
async function addParts(dataEntry, items, how, known) {
  ...
  selectEntry(kids[0]).then(() => {
    /* **1 つの段落に積み上げない** (#284)。#281〜#283 で断り書きが 4 つ増え、
       いちばん大事な一言 (どこを探すか) が 400 字の末尾に埋もれた。
       素人は最初の 1 行で読むのをやめる。事実 → 見当の断り →
       見ていないもの → 次にすること、の 4 段に分ける */
    const lines = [];
    lines.push(
    ...
    /* **「うち」で別の群にぶら下げない** (#284)。ここは長らく
       「`bin` N 件は種類が分からなかったものです。**うち** M 件は…」と続いていたが、
       読まなかったファイルは `bin` とはかぎらない —— 中身が ASCII らしければ
       `txt` になる。`bin` の内訳として語ると、**数の持ち主が入れ替わる** */
    if (unread.n) {
      lines.push(
        `位置表が**先頭 4KB に収まらなかった** ${unread.n} 件は、`
        + "入れ物かどうかを**見ていません** (見ているのは各ファイルの先頭 4KB だけ。"
        + "刻み 8 なら枠 511 個で頭打ちになります)。"
        + "種類の欄にはそれとは別の見当が入っています。この中にテキストがあり得ます。");
    }
    /* **次にすることを、断り書きの山の下に埋めない** (#284)。
    ...
    const next = [];
    if (known === "DFI") {
      next.push("**本文を探すなら `msg` と入れ物**です。");
      if (packed) {
        next.push(`圧縮らしい ${packed} 件 (packed) がありますが、**この作品の本文は`
                  + `圧縮されていません** (英語化パッチの公開ソース一式に伸張処理が`
                  + "ありません)。");
      }
    } else if (packed) {
      next.push(`圧縮らしい ${packed} 件 (packed)。**圧縮を使う作品なら**本文が`
                + "ここにあることがあります (この道具はまだ伸張していません)。"
                + "上の絞り込みに packed と入れると並びます。");
    }
    if (textKinds) {
      next.push(`ただしテキストの入れ物と見た ${textKinds} 件は、`
